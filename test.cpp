...
int main()
{
    ...
    try
    {
        ...
        auto bikeRental = std::make_shared<BikeRental>(true, 16,18, false, 15, 5); // helmet, minimum age, driver age, electric, price per day, insurance fee
        ...
        bikeRental->setHelmet(false);
        assert(bikeRental->getHelmet() == false);
        price = bikeRental->calculatePrice(days, 10);
        assert(price == 55); // 5 days * 10 per day + 10 insurance fee, if helmet is false, it should be -2
    ...
    try
    {
        ...
        rentalService.setRentalStrategy(std::make_shared<BikeRental>(true, 16, 18, false, 15, 5));
